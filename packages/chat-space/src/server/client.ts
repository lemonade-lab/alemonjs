...
import ABodyParser from 'koa-bodyparser'
import ACors from 'koa2-cors'
import AKoa from 'koa'
export class KoaServer {
  ...
  #handlePortConflict(err: { code: string }) {
    if (err.code === 'EADDRINUSE') {
      console.error(`port ${this.#currentPort} occupied, attempting to start a new port...`)
      this.#currentPort++
      this.#size++
      if (this.#size >= 5) {
