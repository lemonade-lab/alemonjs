...
export class QQBotRegistry {
  ...
  constructor(defaultBot?: string, cbp?: ReturnType<typeof cbpPlatform>) {
  ...
  add(botId: string, options: Options) {
    if (!botId) throw new Error('QQ BotId is required');
    if (this.#bots.has(botId)) throw new Error(`Duplicate QQ BotId: ${botId}`);
  ...
  disconnect() {
    for (const { client } of this.#bots.values()) client.disconnect();
    this.#streams.clear();
  }
  ...
  #select(data: any): { entry?: RegisteredBot; error?: string } {
    ...
    if (this.#bots.size === 1) return { entry: this.#bots.values().next().value };
    if (this.#defaultBot) return { entry: this.#bots.get(this.#defaultBot) };
  ...
  async #onApi(data: any, consume: (result: any[]) => void) {
    ...
    await entry.adapter.onApi(data, results => {
      ...
      if (method === 'streamOpen' && result?.streamId) this.#streams.set(String(result.streamId), requested || this.#botIdFor(entry));
      if (streamId && (method === 'streamComplete' || method === 'streamCancel')) this.#streams.delete(String(streamId));
      consume(results);
    });
  }
