      ...
      Body(
        createElement(
          ...
          createElement(
            ...
            P({ className: 'hero-desc' }, '这里会自动列出当前已准备好的主应用与插件。默认入口优先走静态页面，缺失时再回落到应用路由。'),
            Div(
              { className: 'hero-meta' },
              createElement('span', { className: 'hero-pill' }, createElement('strong', null, String(visibleApps.length)), ' 个可访问入口'),
              createElement('span', { className: 'hero-pill' }, '主应用统一在 ', createElement('strong', null, '/app/')),
              createElement('span', { className: 'hero-pill' }, '插件统一在 ', createElement('strong', null, '/apps/<name>/'))
            )
          ),
          createElement(
            'section',
