...
export const ClientControllerOnMessage = (data?: {
  ...
  return (select?: {
    ...
    return {
       ...
       */
      allEmoji: async () => {
        return await ClientVILLA.getAllEmoticons(villa_id, msg_uid)
      }
    }
  }
}
...
export const ClientControllerOnMember = (data?: {
  ...
  return (select?: { guild_id?: string; uid?: string }) => {
    ...
    return {
       ...
       */
      remove: async () => {
        return await ClientVILLA.deleteVillaMember(villa_id, uid)
      },
      /**
       * 身分组
       * @param role_id 身分组编号
      ...
      operate: async (role_id: string, add = true) => {
        return await ClientVILLA.operateMemberToRole(villa_id, {
          role_id,
          uid,
          is_add: add
        })
